...
@section('panel')
    ...
    <div class="row mb-none-30">
        ...
        <div class="col-xl-3 col-lg-5 col-md-5 mb-30">
            ...
            @php
                $imageRank = $user->rank != 0 ? $user->ranks->logo : null;
            @endphp
            @if ($imageRank != null)
                <div class="card b-radius--10 overflow-hidden mt-30 box--shadow1">
                    <div class="card-body text-center">
                        <img src="{{ getImage($imageRank, null, true) }}" alt="UserRank" style="max-width: 200px">
                                ...
                                <select name="rank" class="form-control" id="">
                                    @foreach ($rank as $item)
                                        <option value="{{ $item->id }}"
                                            @if ($user->rank == $item->id) selected @endif>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="input-group mb-3">
                            <select name="rank" class="form-control" id="">
                                @foreach ($rankfounder as $item)
                                    <option value="{{ $item->id }}" @if (($user->rank_founder == 0 ? 1 : $user->rank_founder) == $item->id) selected @endif>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="row mb-none-30">
                ...
                <div class="col-xl-4 col-lg-6 col-sm-6 mb-30">
                    <div class="dashboard-w1 bg--info b-radius--10 box-shadow has--link">
                        <a href="{{ route('admin.log.invest') }}?user={{ $user->id }}" class="item--link"></a>
                        <div class="icon">
                            <i class="la la-money"></i>
                        </div>
                        <div class="details">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <input type="checkbox" data-width="100%" data-height="44px" data-onstyle="-success"
                                    ...
                                    data-off="Subtract POINT" name="act">
                            </div>
                            <div class="form-group col-md-12">
    ...
    <script>
        ...
        (function($) {
            ...
            $('#titik').on('keyup', function() {
                ...
                const num = $(this).val();
                const angka = numberWithCommas(num * plan);
                $("#amountId").val(angka);
            })
